  ...
  useEffect(() => {
    ...
    const fetchDirections = async () => {
      ...
      if (!userLocRaw) {
        ...
        setDirections({
          walking_min: Math.max(1, Math.round(dist / 80)),
          transit_min: Math.max(1, Math.round(dist / 300)),
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', bgcolor: '#F6F8FA', maxWidth: 480, mx: 'auto' }}>
      ...
      <Box sx={{ bgcolor: 'white', px: 2, py: 2, borderBottom: '1px solid #E2E8EE' }}>
        <Typography variant="body2" fontWeight={600} sx={{ mb: 1.5 }}>예상 소요 시간</Typography>
